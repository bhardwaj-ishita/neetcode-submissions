class Solution:
    def isAnagram(self, s: str, t: str) -> bool:
        ...
        """
        """
        #solution in neetcode:
        #a much compact version that's it
        countS, countT = {}, {}
        ...
        # a space complexity optimization
        #so nc is saying if we know there are only small letters, then instead of a making an array a s.len() long
        #which can be n (infinite) we make it always a 26 indices long array
        #basically add for one string and subtrace for another. and then check if the array is all 0
        #but for that remember the function "ord"
        if len(s) != len(t):
            return False
        counter = [0] * 26
        n = len(s)
